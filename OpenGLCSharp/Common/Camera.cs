...
namespace OpenGLCSharp.Common {
    ...
    public class Camera : IDisposable {
        ...
        public Matrix4 GetViewMatrix() { return Matrix4.LookAt( this.Position, this.Position + this._front, this._up ); }

        // Get the projection matrix using the same method we have used up until this point
        public Matrix4 GetProjectionMatrix() { return Matrix4.CreatePerspectiveFieldOfView( this._fov, this.AspectRatio, zNear, zFar ); }

        // Get the ViewProjection matrix using the same method we have used up until this point
        public Matrix4 ViewProjectionMatrix => GetViewMatrix() * GetProjectionMatrix();

        // This function is going to update the direction vertices using some of the math learned in the web tutorials
        private void UpdateVectors() {
            // First the front matrix is calculated using some basic trigonometry
            this._front.X = (float) Math.Cos( this._pitch ) * (float) Math.Cos( this._yaw );
            this._front.Y = (float) Math.Sin( this._pitch );
            this._front.Z = (float) Math.Cos( this._pitch ) * (float) Math.Sin( this._yaw );
        ...
        /// <inheritdoc />
        ~Camera() { Program.ResourceLeak( this.GetType() ); }
